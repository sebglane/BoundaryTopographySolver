...
void Solver<dim>::assemble_system(const bool use_homogeneous_constraints)
{
  ...
  for (const auto &cell : this->dof_handler.active_cell_iterators())
  {
    ...
    for (const auto q: fe_values.quadrature_point_indices())
    {
      ...
      for (const auto i: fe_values.dof_indices())
      {
        const Tensor<1, dim> &test_function_gradient = grad_phi[i];
        const double          test_function_value = phi[i];

        for (const auto j: fe_values.dof_indices())
        {
          double matrix = compute_matrix(grad_phi[j],
        ...
        if (source_term_ptr != nullptr)
          rhs += source_term_values[q] *
                 (test_function_value +
                  delta * advection_field_values[q] * test_function_gradient) ;
