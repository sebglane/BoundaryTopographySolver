 ...
 const double                                                froude_number)
{
  ...
  if (vector_options.use_stress_form)
  {
    ...
    vector_options.present_sym_velocity_gradients = this->get_symmetric_gradients(name, velocity);
  }
  ...
  if (stabilization_flags & (apply_supg|apply_pspg))
  {
    ...
    if (vector_options.use_stress_form)
    {
      ...
      const auto &present_hessians = this->get_hessians(name, velocity);
...
template <int dim>
void ScratchData<dim>::
assign_optional_shape_functions_system_local
(const FEValuesExtractors::Vector  &velocity,
 const FEValuesExtractors::Scalar  &pressure,
...
template <int dim>
void ScratchData<dim>::
assign_optional_shape_functions_rhs_local
(const FEValuesExtractors::Vector  &velocity,
 const FEValuesExtractors::Scalar  &pressure,
 const unsigned int                 q)
{
...
template <int dim>
void ScratchData<dim>::
assign_vector_options_boundary
(const std::string                                          &name,
 const FEValuesExtractors::Vector                           &velocity,
 const FEValuesExtractors::Scalar                           &pressure,
 const double                                                nu,
 ...
 const std::shared_ptr<const TensorFunction<1,dim>>         &background_velocity_ptr)
{
  ...
  else if (vector_options.use_stress_form)
  {
    ...
    // evaluate solution
    const auto &present_pressure_values = this->get_values(name, pressure);
    auto present_velocity_sym_gradients
    = this->get_symmetric_gradients(name, velocity);
...
template <int dim>
void ScratchData<dim>::
assign_optional_shape_functions_system_local_boundary
(const FEValuesExtractors::Vector  &velocity,
 const unsigned int                 q)
{
  const FEValuesBase<dim> &fe_values{this->get_current_fe_values()};
