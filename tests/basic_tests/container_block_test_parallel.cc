/*
 * linear_algebra_container_02.cc
 *
 ...
 const unsigned int degree = 1)
{
  ConditionalOStream pcout(std::cout,
                           bool(Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0));
...
int main(int argc, char *argv[])
{
  try
  {
    ...
    using TrilinosContainer = typename SolverBase::
        LinearAlgebraContainer<TrilinosWrappers::MPI::Vector,
                               TrilinosWrappers::SparseMatrix,
                               TrilinosWrappers::SparsityPattern>;

    for (std::size_t i=1; i<9; ++i)
      TestSpace::test_container<2, TrilinosContainer>(i);

    for (std::size_t i=1; i<9; ++i)
      TestSpace::test_container<3, TrilinosContainer>(i);
